...
parser.add_argument('--style_folder', type=str, help="style image folder")
...
if __name__ == '__main__': # for multi-process in windows must have this line
    with torch.no_grad():
        ...
        if opts.trainer == 'MUNIT':
            ...
            for i, (images, names) in enumerate(zip(data_loader, image_names)):
                ...
                # if basename.split('.')[-1] == ''
                # style_name = os.path.join(opts.style_folder, basename.split('.')[0]+'.jpg')
                style_name = os.path.join(opts.style_folder, basename)
                style_image = Variable(transform(Image.open(style_name).convert('RGB')).unsqueeze(0).cuda())
